...
void HexArray1D::print(){
    ...
    for(int i = 0; i < getSize(); i++){
        ...
        for(int j = 0; j < getSize(); j++){
            cout << hexCells[i*getSize() + j].getState() << " ";
        }
        cout << endl;
    }
}
/*reads a game from file*/
void HexArray1D::readFromFile(int& player_turn, const std::string& filename){
    ifstream fin;
    string s;
    fin.open(filename);
    if(fin){
        ...
        for(int i = 0, k = 0; i < getSize(); i++){
            for(int j = 0; j < getSize(); j++, k++){
                hexCells[i*getSize() + j].setState(static_cast<cell_state>(s[k]));
            }
        }
    }
    else{ 
...
void HexArray1D::writeToFile(const int& player_turn, const string& filename) const{
    ofstream fout;
    fout.open(filename);
    if(fout){
        ...
        for(int i = 0; i < getSize(); i++){
            for(int j = 0; j < getSize(); j++){
                fout << hexCells[i*getSize() + j].getState();
            }
        }
    }
    else{ 
...
void HexArray1D::reset(){
    for(int i = 0; i < getSize(); i++){
        for(int j = 0; j < getSize(); j++){
            if(hexCells[i*getSize() + j].getState() != Empty) hexCells[i*getSize() + j].setState(Empty);
        }
    }
}
...
void HexArray1D::setSize(const int& size){
    ...
    for(int i = 0; i < size; i++){
        for(int j = 0; j < size; j++){
            ...
            hexCells[i*size + j].setColumn(j); 
        }
    }
    reset();
}
/*plays a single move for players*/
void HexArray1D::play(const Cell& coordinates){
    hexCells[coordinates.getRow()*getSize() + coordinates.getColumn()].setState(static_cast<cell_state>(coordinates.getState()));
    moveNum++;
}
/*plays a single move for computer*/
void HexArray1D::play(){
    if(check_cell_state(get_pastx(), get_pasty() + 1) == true){
        hexCells[(get_pastx() - 1)*getSize() + static_cast<int>(get_pasty()) - 96].setState(Bot);
        ...
        hexCells[(get_pastx() - 2)*getSize() + static_cast<int>(get_pasty()) - 96].setState(Bot);
        ...
        hexCells[(get_pastx())*getSize() + static_cast<int>(get_pasty()) - 97].setState(Bot);
        cout << "\nComputer's move: " << get_pasty() << get_pastx() + 1 << endl;
        last_move.setRow(get_pastx());
        last_move.setColumn(static_cast<int>(get_pasty()) - 97);
    }
    else if(check_cell_state(get_pastx() - 1, get_pasty()) == true){
        hexCells[(get_pastx() - 2)*getSize() + static_cast<int>(get_pasty()) - 97].setState(Bot);
        cout << "\nComputer's move: " << get_pasty() << get_pastx() - 1 << endl;
        last_move.setRow(get_pastx() - 2);
        last_move.setColumn(static_cast<int>(get_pasty()) - 97);
    }
    else if(check_cell_state(get_pastx() + 1, get_pasty() - 1) == true){
        hexCells[(get_pastx())*getSize() + static_cast<int>(get_pasty()) - 98].setState(Bot);
        cout << "\nComputer's move: " << static_cast<char>(get_pasty() - 1) << get_pastx() + 1 << endl;
        last_move.setRow(get_pastx());
        last_move.setColumn(static_cast<int>(get_pasty()) - 98);
    }
    else if(check_cell_state(get_pastx(), get_pasty() - 1) == true){
        hexCells[(get_pastx() - 1)*getSize() + static_cast<int>(get_pasty()) - 98].setState(Bot);
        cout << "\nComputer's move: " << static_cast<char>(get_pasty() - 1) << get_pastx() << endl;
        last_move.setRow(get_pastx() - 1);
        last_move.setColumn(static_cast<int>(get_pasty()) - 98);
    }
        ...
        for(int i = 0; i < getSize(); i++){
            for(int j = 0; j < getSize(); j++){
                if(check_cell_state(i, j) == true){
                    hexCells[i*getSize() + j].setState(Bot);
                    last_move.setRow(i);
...
bool HexArray1D::isWinner_P1(const int& x, const int& y, int rotation){
    if(hexCells[x*getSize() + y].getState() != Player1){
        return false;
    }
    else if(hexCells[x*getSize() + y].getState() == Player1 && y == getSize() - 1){
        hexCells[x*getSize() + y].setState(static_cast<cell_state>(Player1 - 32));
        return true;
    }
    else{
...
bool HexArray1D::isWinner_P2(const int& x, const int& y, int rotation){
    if(hexCells[x*getSize() + y].getState() != Player2){
        return false;
    }
    else if(hexCells[x*getSize() + y].getState() == Player2 && x == getSize() - 1){
        hexCells[x*getSize() + y].setState(static_cast<cell_state>(Player2 - 32));
        return true;
    }
    else{
        //function determines the coordinates of the previous node by finding the opposite rotation of current one and records these coordinates so that function doesn't check the same coordinates over and over again and this prevents the function from getting into an infinite loop
...
bool HexArray1D::check_cell_state(const int& x, const char& y) const{
    if(x - 1 >= 0 && x - 1 <= getSize() - 1 && static_cast<int>(y) - 97 >= 0 && static_cast<int>(y) - 97 <= getSize() - 1){
        if(hexCells[(x - 1)*getSize() + static_cast<int>(y) - 97].getState() == Empty){
            return true;
        }
    }
    return false;
...
bool HexArray1D::isConnected(const cell_state& player){
    int check1 = 0, check2 = 0;
    if(player == Player1){
        for(int i = 0; i < getSize() && check1 == 0; i++){
            if(hexCells[i*getSize()].getState() == player){
                check1++;
            }
        }
        for(int i = 0; i < getSize() && check2 == 0; i++){
            if(hexCells[i*getSize() + getSize() - 1].getState() == player){
                check2++;
            }
        }
    }
        ...
        for(int i = 0; i < getSize() && check1 == 0; i++){
            if(hexCells[i].getState() == player){
                check1++;
            }
        }
        for(int i = 0; i < getSize() && check2 == 0; i++){
            if(hexCells[(getSize() - 1)*getSize() + i].getState() == player){
                check2++;
            }
        }
    }
...
void HexArray1D::find_beginning(const cell_state& player){
    if(player == Player1){
...
HexArray1D::HexArray1D(const int& s) : AbstractHex(s){
    setSize(s);
}
/*constructor with size and gamestatus arguments*/
HexArray1D::HexArray1D(const int& s, const int& gm) : AbstractHex(s, gm){
    setSize(s);
}
/*copy constructor*/
HexArray1D::HexArray1D(const HexArray1D& other){
    hexCells = (Cell*)malloc(other.boardSize);
    p1_first_coordinates = other.p1_first_coordinates;
    p2_first_coordinates = other.p2_first_coordinates;
    p1_past_coordinates = other.p1_past_coordinates;
    p2_past_coordinates = other.p2_past_coordinates;
